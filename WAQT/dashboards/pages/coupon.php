...
class CouponTable {
    ...
    private function renderRow($row, $isDeleted = false, $isFinished = false) {
        ...
        if ($isDeleted) {
            ...
            echo "<td class='align-middle text-center'><button class='badge badge-sm bg-gradient-success border-0' onclick='showEditCouponDialog(\"{$row['coupon_id']}\", \"" . htmlspecialchars($row['coupon_value']) . "\", \"" . htmlspecialchars($row['start_date']) . "\", \"" . htmlspecialchars($row['end_date']) . "\", \"" . htmlspecialchars($row['usage_limit']) . "\", \"" . htmlspecialchars($row['coupon_name']) . "\")'>Edit</button></td>";
...
echo "<b>Your search: </b>" . htmlspecialchars($search) . "<br><br>";
